                 ...
                 (when (stringp (first current-expr))
                   (let ((operator (first current-expr)))
                     (cond
                       ...
                       ((base:form-head-name-p operator "DEFUN")
                       ...
                       ((base:form-head-name-p operator "DEFMETHOD")
                        (when (>= (length current-expr) 3)
                       ...
                       ((base:form-head-name-p operator "LAMBDA")
                       ...
                       ((base:form-head-name-p operator "DEFMACRO")
                        (when (>= (length current-expr) 3)
                          (let* ((lambda-list (third current-expr))
                            ...
                            (when (and result
                              ...
                              (push (make-instance 'violation:violation
                                    ...
                                    violations)))))
                       ;; flet and labels
                       ((or (base:form-head-name-p operator "FLET")
                            (base:form-head-name-p operator "LABELS"))
                        (when (>= (length current-expr) 2)
                                  ...
                                  (let* ((lambda-list (second binding))
                                    ...
                                    (when (and result
                                      ...
                                      (push (make-instance 'violation:violation
                                            ...
                                            violations)))))))))
                       ;; destructuring-bind
                       ((base:form-head-name-p operator "DESTRUCTURING-BIND")
                        (when (>= (length current-expr) 3)
                          (let* ((lambda-list (second current-expr))
